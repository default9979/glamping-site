...
export default function BookingForm() {
  ...
  const selectedHouse = useMemo(
    () => houses.find((h) => h.name === house) ?? houses[0],
    [house],
  )
    ...
    <section id="booking" className="relative bg-surface px-6 py-28">
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <div className="grid items-stretch gap-6 lg:grid-cols-2">
          ...
          >
            <p className="mb-3 text-sm text-dim">Адрес: {selectedHouse.address}</p>
            <YandexMap key={selectedHouse.id} house={selectedHouse} />
          </motion.div>
        </div>
      </div>
    </section>
